...
void GenCode::visit(FunDec* f) {
    ...
    for (auto a:f->cuerpo->vardecs->vardecs)
    if(a->type=="char") {
        a->accept(this);
    }
...
void caberaString(VarDec* stm) {
    if(stm->type=="char") {
        out<<stm->vars.back()->id<<": ";
        if(stm->vars.back()->dimList.back()==nullptr) {
            out<<stm->vars.back()->iv->value->accept(this).string_value;
            return;
        }
    }
}
